...
void Tetris::update(double time) {
...
void Tetris::passGridBuffer() {
  std::vector<glm::vec3> gridColors(gridWidth * gridHeight + 16 * 2);
  for(int i = 0; i < gridWidth * gridHeight; ++i)
      gridColors[i] = Tetris::pieceColors[this->grid[i]];
...
const glm::vec3 Tetris::pieceColors[Mino::numPieces + 2] = {
